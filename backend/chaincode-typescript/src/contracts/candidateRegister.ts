...
export class CandidateRegisterContract extends Contract {
    @Transaction()
    public async InitCandidatesLedger(ctx: Context): Promise<void> {
        const candidates: Candidate[] = [
            {
                idCandidate: 'candidate1',
                name: 'John',
                idParty: 'party1',
                docType: 'candidate',
                votingCode: 101,
        ...
        for (const candidate of candidates) {
            ...
            console.info(`Candidate ${candidate.name} registered.`);
        }
    }
